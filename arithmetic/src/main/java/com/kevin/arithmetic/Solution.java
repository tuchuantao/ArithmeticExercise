...
//        System.out.print(result);
    ...
    public static List<List<String>> solveNQueens(int n) {
        ...
        for (int col = 0; col < n; col++) {
            findNQueen(new int[n][n], 0, col, result, new ArrayList());
        }
        return result;
    }

    public static void findNQueen(int[][] chessboard, int row, int col, List<List<String>> result, List<String> singResult) {
        addQueen(chessboard, row, col, singResult);

        if (singResult.size() == chessboard.length) {
            result.add(new ArrayList<>(singResult));
            removeQueen(chessboard, row, col, singResult);
            return;
        }
        ...
        for (int i = 0; i < chessboard.length; i++) {
            if (chessboard[row + 1][i] == 0) {
                findNQueen(chessboard, row + 1, i, result, singResult);
            }
        }
        removeQueen(chessboard, row, col, singResult);
    }

    public static void removeQueen(int[][] chessboard, int row, int col, List<String> singResult) {
        for (int i = 0; i < chessboard.length; i++) {
            chessboard[row][i] -= 1;
            chessboard[i][col] -= 1;
            if (col + i < chessboard.length) {
        ...
        singResult.remove(singResult.size() - 1);
    }

    public static void addQueen(int[][] chessboard, int row, int col, List<String> singResult) {
        ...
        singResult.add(builder.toString());
    }
