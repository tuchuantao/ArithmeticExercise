package com.kevin.arithmetic.leetcode;

import java.util.Deque;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.Map;

/**
 * Created by tuchuantao on 2022/1/25
 * Desc: 实现一个魔法字典
 */
public class Num676 {
  /**
   * 设计一个使用单词列表进行初始化的数据结构，单词列表中的单词 互不相同 。 如果给出一个单词，请判定能否只将这个单词中一个字母换成另一个字母，使得所形成的新单词存在于你构建的字典中。
   * 实现 MagicDictionary 类：
   * MagicDictionary() 初始化对象
   * void buildDict(String[]dictionary) 使用字符串数组dictionary 设定该数据结构，dictionary 中的字符串互不相同
   * bool search(String searchWord) 给定一个字符串 searchWord ，判定能否只将字符串中 一个 
   * 字母换成另一个字母，使得所形成的新字符串能够与字典中的任一字符串匹配。如果可以，返回 true ；否则，返回 false 。
   * 
   * 示例：
   * 输入
   * ["MagicDictionary", "buildDict", "search", "search", "search", "search"]
   * [[], [["hello", "leetcode"]], ["hello"], ["hhllo"], ["hell"], ["leetcoded"]]
   * 输出
   * [null, null, false, true, false, false]
   *
   * 解释
   * MagicDictionary magicDictionary = new MagicDictionary();
   * magicDictionary.buildDict(["hello", "leetcode"]);
   * magicDictionary.search("hello"); // 返回 False
   * magicDictionary.search("hhllo"); // 将第二个 'h' 替换为 'e' 可以匹配 "hello" ，所以返回 True
   * magicDictionary.search("hell"); // 返回 False
   * magicDictionary.search("leetcoded"); // 返回 False
   *
   * 提示：
   * 1 <=dictionary.length <= 100
   * 1 <=dictionary[i].length <= 100
   * dictionary[i] 仅由小写英文字母组成
   * dictionary 中的所有字符串 互不相同
   * 1 <=searchWord.length <= 100
   * searchWord 仅由小写英文字母组成
   * buildDict 仅在 search 之前调用一次
   * 最多调用 100 次 search
   *
   * 来源：力扣（LeetCode）
   * 链接：https://leetcode-cn.com/problems/implement-magic-dictionary
   * 著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
   */
  class MagicDictionary { // 字典树
    Node header;

    /** Initialize your data structure here. */
    public MagicDictionary() {
      header = new Node();
    }

    public void buildDict(String[] dictionary) {
      Node cur;
      for (String str : dictionary) {
        cur = header;
        for (char c : str.toCharArray()) {
          if (cur.childMap.get(c) == null) {
            cur.childMap.put(c, new Node());
          }
          cur = cur.childMap.get(c);
        }
        cur.isEnd = true;
      }
    }

    public boolean search(String searchWord) {
      int len = searchWord.length();
      Deque<Node> deque = new LinkedList();
      for (int i = 0; i < len; i++) {
        deque.clear();
        deque.addLast(header);
        int index = 0;
        while (index < len && !deque.isEmpty()) {
          int size = deque.size();
          for (int j = 0; j < size; j++) {
            Node node = deque.removeFirst();
            if (index == i) {
              for (Map.Entry<Character, Node> entry: node.childMap.entrySet()) {
                if (entry.getKey() == searchWord.charAt(i)) {
                  continue;
                }
                if (index == len - 1 && entry.getValue().isEnd) {
                  return true;
                }
                deque.addLast(entry.getValue());
              }
            } else if (node.childMap.get(searchWord.charAt(index)) != null) {
              if (index == len - 1 && node.childMap.get(searchWord.charAt(index)).isEnd) {
                return true;
              }
              deque.addLast(node.childMap.get(searchWord.charAt(index)));
            }
          }
          index++;
        }
      }
      return false;
    }

    private class Node {
      Map<Character, Node> childMap = new HashMap();
      boolean isEnd = false;
    }
  }
}
